...
export const create = ({ bodymen: { body: { email, link } } }, res, next) =>
  User.findOne({ email })
    ...
    .then((reset) => {
      ...
      const content = `
        Hey, ${user.name}.<br><br>
        You requested a new password for your my_web_server account.<br>
        ...
        &mdash; my_web_server Team
      `
      return sendMail({ toEmail: email, subject: 'my_web_server - Password Reset', content })
    })
